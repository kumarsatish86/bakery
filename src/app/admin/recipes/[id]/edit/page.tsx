...
export default function EditRecipe({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchParams = async () => {
      ...
      await fetchRecipe(resolvedParams.id);
    };
    fetchParams();
    fetchProducts();
  }, [params]);
  ...
  const fetchRecipe = async (id: string) => {
    try {
      ...
      if (response.ok) {
        ...
        setError(`Failed to fetch recipe: ${errorData.message || 'Recipe not found'}`);
      }
    } catch (error) {
      console.error('Error fetching recipe:', error);
      setError('An error occurred while fetching recipe');
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        setError(errorData.message || 'Failed to update recipe');
      }
    } catch (error) {
      console.error('Error updating recipe:', error);
      setError('An error occurred while updating recipe');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  if (isLoadingData) {
    return (
      <AdminLayout 
        title="Edit Recipe" 
        subtitle="Update recipe information"
      >
    ...
    return (
      ...
      >
        <div className="text-center py-12">
          <div className="text-gray-500 text-lg">Recipe not found</div>
  ...
  return (
    <AdminLayout 
      title="Edit Recipe" 
      subtitle={`Update recipe ${recipe.name}`}
    >
