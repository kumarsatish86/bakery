...
export default function EditWarehouse({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchParams = async () => {
      ...
      await fetchWarehouse(resolvedParams.id);
    };
    fetchParams();
  }, [params]);
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData({
          ...
          zipCode: data.warehouse.zipCode || '',
        ...
        setError(`Failed to fetch warehouse: ${errorData.message || 'Warehouse not found'}`);
      }
    } catch (error) {
      console.error('Error fetching warehouse:', error);
      setError('An error occurred while fetching warehouse');
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        setError(errorData.message || 'Failed to update warehouse');
      }
    } catch (error) {
      console.error('Error updating warehouse:', error);
      setError('An error occurred while updating warehouse');
    } finally {
      setIsLoading(false);
    }
  };
    ...
    return (
      ...
      >
        <div className="text-center py-12">
          <div className="text-gray-500 text-lg">Warehouse not found</div>
