...
export default function EditDelivery({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    const fetchParams = async () => {
      ...
      setDeliveryId(resolvedParams.id);
      await fetchDelivery(resolvedParams.id);
    };
    fetchParams();
    fetchOrders();
  }, [params]);
    ...
    try {
      ...
      if (response.ok) {
        ...
        setFormData({
          ...
          scheduledDate: data.delivery.scheduledDate ? new Date(data.delivery.scheduledDate).toISOString().slice(0, 16) : '',
          ...
          vehicleNumber: data.delivery.vehicleNumber || '',
          trackingNumber: data.delivery.trackingNumber || '',
          status: data.delivery.status || 'SCHEDULED'
        });
        setError(''); // Clear any previous errors
      } else {
        const errorData = await response.json();
        setError(`Failed to fetch delivery: ${errorData.message || 'Delivery not found'}`);
      }
    } catch (error) {
  ...
  const handleOrderChange = (orderId: string) => {
    ...
    if (selectedOrder) {
      setFormData(prev => ({
        ...prev,
        orderId,
        deliveryAddress: selectedOrder.customer.address || prev.deliveryAddress,
        city: selectedOrder.customer.city || prev.city,
        phone: selectedOrder.customer.phone || prev.phone
      }));
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        setError(errorData.message || 'Failed to update delivery');
      }
    } catch (error) {
  ...
  if (isLoadingData) {
    return (
      <AdminLayout 
        title="Edit Delivery" 
        subtitle="Update delivery information"
      >
    ...
    return (
      ...
      >
        <div className="text-center py-12">
          <div className="text-gray-500 text-lg">Delivery not found</div>
  ...
  return (
    <AdminLayout 
      title="Edit Delivery" 
      subtitle={`Update delivery ${delivery.order.orderNumber}`}
    >
