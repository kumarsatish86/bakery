...
export default function CustomerManagement() {
  ...
  const b2bCustomers = customers.filter(customer => customer.customerType === 'B2B').length;
  const communityCustomers = customers.filter(customer => customer.customerType === 'COMMUNITY').length;
  ...
  const fetchCustomers = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setError(`Failed to fetch customers: ${errorData.message || 'Unknown error'}`);
      }
    } catch (error) {
  ...
  const updateCustomerType = async (customerId: string, newType: string) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`/api/customers/${customerId}/type`, {
        method: 'PUT',
        headers: {
      ...
      if (response.ok) {
        setCustomers(customers.map(customer => 
          customer.id === customerId ? { ...customer, customerType: newType } : customer
        ));
        setError(''); // Clear any previous errors
      } else {
        const errorData = await response.json();
        setError(`Failed to update customer type: ${errorData.message || 'Unknown error'}`);
      }
    } catch (error) {
      console.error('Error updating customer type:', error);
      setError('An error occurred while updating customer type');
    }
  };
  ...
  const toggleCustomerStatus = async (customerId: string) => {
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        setError(`Failed to toggle customer status: ${errorData.message || 'Unknown error'}`);
      }
    } catch (error) {
      console.error('Error toggling customer status:', error);
      setError('An error occurred while toggling customer status');
    }
  };
  ...
  const handleDeleteCustomer = async (customerId: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        setError(`Failed to delete customer: ${errorData.message || 'Unknown error'}`);
      }
    } catch (error) {
      ...
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-6">
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm border">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm font-medium text-gray-600">B2B</p>
                <p className="text-2xl font-bold text-gray-900">{b2bCustomers}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow-sm border">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">{communityCustomers}</p>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-sm text-gray-600">Manage customer accounts and their information</p>
          </div>
          <button 
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {paginatedCustomers.map((customer) => (
                    <tr key={customer.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center">
                          <span className="text-lg mr-2">{getCustomerTypeIcon(customer.customerType)}</span>
