...
export default function ProductManagement() {
  ...
  // Helper functions
  const getTotalStock = (inventory: Array<{id: string; quantity: number}>) => {
    return inventory.reduce((sum, inv) => sum + inv.quantity, 0);
  };

  const getStockStatus = (product: Product) => {
    const totalStock = getTotalStock(product.inventory);
    if (totalStock <= product.minStockLevel) return 'low';
    if (product.maxStockLevel && totalStock >= product.maxStockLevel) return 'high';
    return 'normal';
  };
  ...
  const lowStockProducts = products.filter(product => getStockStatus(product) === 'low').length;
  const totalStockUnits = products.reduce((sum, product) => sum + getTotalStock(product.inventory), 0);
  ...
  const filteredProducts = products.filter(product => {
    ...
    const matchesStock = stockFilter === 'all' || 
                        ...
                        (stockFilter === 'high' && getStockStatus(product) === 'high');
  ...
  const toggleProductStatus = async (productId: string) => {
    try {
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        setError(`Failed to toggle product status: ${errorData.message || 'Unknown error'}`);
      }
    } catch (error) {
                    ...
                    return (
                      <tr key={product.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          ...
                          <div className={`text-xs px-2 py-1 rounded-full ${getStockStatusColor(stockStatus)}`}>
